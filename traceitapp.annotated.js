 ...
 */
angular.module('prettyprint', []).directive('prettyPrint', [
  '$filter',
  function ($filter) {
    'use strict';
    return {
...
/* jshint devel:true */
angular.module('traceitapp.home', []).controller('HomeCtrl', [
  '$scope',
  '$rootScope',
  '$state',
  '$location',
  ...
  function ($scope, $rootScope, $state, $location, $anchorScroll) {
    ...
    $scope.catListItems = [
      {
        name: 'Friendly and content',
        value: 'c1'
      },
      {
        name: 'Non-threatening, unsure',
        value: 'c2'
      },
      {
        name: 'Derisive',
      ...
      {
        name: 'Friendly but unsure',
        value: 'c4'
      },
      {
        name: 'Amicable, not fearful or aggressive',
        value: 'c5'
      },
      {
      ...
      {
        name: 'Excited, angry or irritable',
        value: 'c11'
      },
      {
        name: 'Alert, interested',
        value: 'c12'
      }
    ];
    ...
    $scope.traceRandom = function () {
      if (!$scope.box3) {
        ...
        $scope.traceOptEx2.stroke = '#' + (Math.random() * 16777215 << 0).toString(16);
        $scope.traceOptEx2.fill = '#' + Math.floor(Math.random() * 16777215).toString(16);
...
(function (module) {
  ...
  module.run([
    '$templateCache',
    function ($templateCache) {
      $templateCache.put('/traceitapp/common/services/modal-service/modalService.html', '<div class="modal-header"><h3>{{modalOptions.headerText}}</h3></div><div class="modal-body"><p>{{modalOptions.bodyText}}</p></div><div class="modal-footer"><button type="button" class="btn" data-ng-click="modalOptions.close()">{{modalOptions.closeButtonText}}</button> <button class="btn btn-primary" data-ng-click="modalOptions.ok();">{{modalOptions.actionButtonText}}</button></div>');
    }
  ]);
}());
angular.module('traceitapp', [
  'traceitapp-templates',
  'traceitapp.home',
  'angular-traceit',
  'prettyprint',
  'ui.router',
  ...
  function ($stateProvider, $urlRouterProvider) {
    ...
    $stateProvider.state('traceit-start', {
      url: '/home',
 ...
 * Directives
 */
angular.module('traceitapp.home').directive('wsize', [
  '$window',
  function ($window) {
    'use strict';
 ...
 * Filters
 */
angular.module('prettyprint').filter('prettyprint', function () {
...
angular.module('traceitapp.home').service('modalService', [
  '$modal',
  function ($modal) {
    var modalDefaults = {
        backdrop: true,
 ...
 * Services
 */
angular.module('prettyprint').factory('filterByKeys', function () {
  return {
